...
namespace MyMediaLite.Eval
{
    ...
    public static class Ratings2
    {
        /// <summary>the evaluation measures for item prediction offered by the class</summary>
        /// <remarks>
        /// The evaluation measures currently are:
        /// <list type="bullet">
        ///   <item><term>ATOP</term><description>area under the recall curve</description></item>
        ///   <item><term>prec@N</term><description>precision at N</description></item>
        ///   <item><term>recall@N</term><description>recall at N</description></item>
        ///   <item><term>NDCG@N</term><description>normalizad discounted cumulative gain at N</description></item>
        /// </list>
            ...
            IRatings training)
        {
            ...
            switch (candidate_item_mode)
            {
                ...
                case CandidateItems.LONGTAIL:
                    ...
                    {
                        ItemPopularity recommender = new ItemPopularity();
                        recommender.Ratings = training;
                        recommender.Train();
        ...
        static public Dictionary<string, double> Evaluate(
            ...
            CandidateItems candidate_item_mode = CandidateItems.UNION,            
            ...
            int n = -1)
        {
            ...
            IList<int> all_items = Candidates(candidate_items, CandidateItems.UNION, test, training);

            var result = new Dictionary<string, double>();
            foreach (string measure in Ratings2.Measures)
                result[measure] = 0d;

            // if corresponding user-item pairs are rated by the user, assign true
            var training_user_matrix = new MyMediaLite.DataType.SparseBooleanMatrix();
            //
            for (int index = 0; index < training.Count; index++)
                training_user_matrix[training.Users[index], training.Items[index]] = true;

            // ground truth (items with the highest ratings(i.e. five star) are taken as relevant items)
            var test_user_matrix = new MyMediaLite.DataType.SparseBooleanMatrix();
            for (int index = 0; index < test.Count; index++)
            {
                float test_rating = test[test.Users[index], test.Items[index]];
                test_user_matrix[test.Users[index], test.Items[index]] = true;
            }
            int num_users = 0;
            foreach (int user_id in test_users)
            {
                // Check the testing data (if # testing data is zero, skip it.)
                var correct_items = new HashSet<int>(test_user_matrix[user_id]);
                if (candidate_item_mode == CandidateItems.LONGTAIL)
                    correct_items.IntersectWith(all_items);
                else
                    correct_items.IntersectWith(candidate_items);
                if (correct_items.Count == 0)
                    continue;

                // Check the training data (if # traing data is equal to # candidate items, skip it.)

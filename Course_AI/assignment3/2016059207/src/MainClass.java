...
public class MainClass {
	...
	public static void makeOutput(int N, String path) { // 이 함수 내에서 과제로 주어진 알고리즘이 수행됩니다
		...
		Genetic g = new Genetic(N); // Genetic 알고리즘입니다
		long beforeTime = System.currentTimeMillis();
		g.start(); // 이 함수를 호출하면 결과에 해당하는 문자열이 Genetic 클래스 내에 생성됩니다
		long afterTime = System.currentTimeMillis();
		
		try {
			outputStream = new PrintWriter(new FileOutputStream(OsFilePath + "\\result" + str_N + ".txt"));
			
			outputStream.print(">Genetic Algorithm(cnt=");
			outputStream.print(g.cnt_generation); outputStream.println(')');
			if(g.findAns) {
			outputStream.println(g.make_result()); // make_result함수가 resultN.txt에 쓸 문자열을 반환해줍니다

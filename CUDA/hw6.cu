...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    if (col >= bound || row >= bound) { return; }

    float max = -99999.0;
    for (int i = 0; i < filter_size; i++) {
        for (int j = 0; j < filter_size; j++) {
            float temp = input[(row * filter_size + i) * input_size + (col * filter_size + j)];
            if (max < temp) max = temp;
        }
    }
    output[row * bound + col] = max;
}
...
__global__ void gemm(float *a, float *b, float *c, const float alpha, const float beta, float *output, const int input_size){
    ...
    int bound = ceil((float)input_size/TILE_WIDTH);

    // when the matrix sizes are multiple of TILE_WIDTH
    if (bound == input_size/TILE_WIDTH) {
        ...
        for (int p = 0; p < bound; ++p) {
            s_a[ty][tx] = a[input_size*row + tx + TILE_WIDTH*p];
            s_b[ty][tx] = b[col + input_size*(ty + TILE_WIDTH*p)];
            __syncthreads();
...
int main(int argc, char **argv) {
    ...
    float *a = (float*)malloc(sizeof(float)*input_size*input_size);
    float *b = (float*)malloc(sizeof(float)*input_size*input_size);
    float *c = (float*)malloc(sizeof(float)*input_size*input_size);

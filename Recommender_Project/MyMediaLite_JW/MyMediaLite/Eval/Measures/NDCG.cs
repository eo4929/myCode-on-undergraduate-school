...
namespace MyMediaLite.Eval.Measures
{
    ...
    public static class NDCG
    {
        ...
        /// <summary>Compute the NDCG at N of a list of ranked items at several N</summary>
        /// <param name="ranked_items">a list of ranked item IDs, the highest-ranking item first</param>
        /// <param name="correct_items">a collection of positive/correct item IDs</param>
        /// <param name="ns">the cutoff positions in the list</param>
        /// <returns>the NDCG at N for the given data at the different positions N</returns>
        public static Dictionary<int, double> NDCGAt(
            IList<int> ranked_items,
            ICollection<int> correct_items,
            IList<int> ns)
        {
            var ndcg_at_n = new Dictionary<int, double>();
            foreach (int n in ns)
                ndcg_at_n[n] = NDCGAt(ranked_items, correct_items, n);
            return ndcg_at_n;
        }

        /// <summary>Compute the NDCG at N of a list of ranked items</summary>
        /// <param name="ranked_items">a list of ranked item IDs, the highest-ranking item first</param>
        /// <param name="correct_items">a collection of positive/correct item IDs</param>
        /// <param name="n">the cutoff position in the list</param>
        /// <returns>the NDCG at N for the given data</returns>
        public static double NDCGAt(IList<int> ranked_items, ICollection<int> correct_items, int n)
        {
            double dcg = 0;
            double idcg = ComputeIDCG(Math.Min(correct_items.Count, n));

            int size = Math.Min(ranked_items.Count, n);

...
{
	if(*pRoot == NULL) // 노드가 없다면
	{
		*pRoot = MakeBTreeNode();
		SetData(*pRoot,data);
	}
	else if(data < GetData(*pRoot)) // 입력 데이터가 루트 노드 데이터보다 작다면
	{
		BSTInsert(&((*pRoot)->left),data); // 왼쪽 서브트리 노드에 데이터 대입
		*pRoot = Rebalance(pRoot); // 리벨런싱 수행
	}
	else if(data > GetData(*pRoot)) // 입력 데이터가 루트 노드 데이터보다 크다면
	{
		BSTInsert(&((*pRoot)->right),data); // 오른쪽 서브트리 노드에 데이터 대입
		*pRoot = Rebalance(pRoot); // 리벨런싱 수행
	}
	else
	{
		return NULL; // 데이터 중복 x
	}

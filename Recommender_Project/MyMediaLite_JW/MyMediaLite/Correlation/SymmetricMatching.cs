...
{
    public sealed class SymmetricMatching : SymmetricCorrelationMatrix, IRatingCorrelationMatrix
    {
        public SymmetricMatching(int num_entities) : base(num_entities) { }
        ...
        public void ComputeCorrelations(IRatings ratings, EntityType entity_type)
        {
            ...
            for (int i = 0; i < num_entities; i++)
                ...
                {
                    double denominator = (entity_type == EntityType.USER) ? ratings.MaxItemID : ratings.MaxUserID;

                    double numerator = overlaps_rating[i, j] + (denominator - count[i] - count[j] + overlaps[i, j]);

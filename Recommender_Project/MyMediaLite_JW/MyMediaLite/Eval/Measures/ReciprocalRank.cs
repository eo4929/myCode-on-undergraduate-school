...
namespace MyMediaLite.Eval.Measures
{
	...
	public static class ReciprocalRank
	{
        ...
        public static Dictionary<int, double> ComputeAt(IList<int> ranked_items, ICollection<int> correct_items, IList<int> ns)
        {       
            var mrr_at_n = new Dictionary<int, double>();
            foreach (int n in ns)
                mrr_at_n[n] = 0;
            for (int rank = 0; rank < ns[ns.Count-1]; rank++)
            {
                if (correct_items.Contains(ranked_items[rank]))
                {
                    foreach (int n in ns)
                    {
                        if (n < rank)
